...
public class ProjectsAtAGlanceFrame extends JFrame implements MouseListener, ActionListener
{
...
private JTable theprojectTable;
private TheProjectTableModel theprojectModel;
private JPanel theprojectTitlePane;
...
theprojectModel = new TheProjectTableModel(s);
theprojectTable = new JTable(theprojectModel);
theprojectTable.setColumnSelectionAllowed(false);
theprojectTable.setRowSelectionAllowed(false);
theprojectTable.addMouseListener(this);
theprojectTable.getTableHeader().setReorderingAllowed(false);
// make it so that the user can make each column disappear if they want:
numCols = theprojectTable.getColumnCount();
for(int i=0; i<numCols; i++)
{
theprojectTable.getColumnModel().getColumn(i).setMinWidth(0);
}
...
// Create panes:
JScrollPane theprojectPane = new JScrollPane(theprojectTable);

// Table headers:
theprojectTitlePane = new JPanel();
theprojectTitlePane.add(new JLabel("TheProjects:"));
...
mainPane.add(theprojectTitlePane);
mainPane.add(theprojectPane);
...
{
if(me.getComponent().equals(theprojectTable)) // correct table
{
createPopupMenu(theprojectTable, p);
}
}
}
...
if(model instanceof TheProjectTableModel)
{
column = selectedTable.getColumnModel().getColumn(((TheProjectTableModel)selectedTable.getModel()).getColumnIndex(itemText));
}
if(column != null)
{
...
theprojectModel.update();
if (!state.getClock().isStopped()) { // game not over
theprojectTable.getColumnModel().getColumn(theprojectModel.getColumnIndex("CurrentIteration")).setCellRenderer(rightAlignRenderer);
theprojectTable.getColumnModel().getColumn(theprojectModel.getColumnIndex("TimeElapsed")).setCellRenderer(rightAlignRenderer);
theprojectTable.getColumnModel().getColumn(theprojectModel.getColumnIndex("TimeAllotted")).setCellRenderer(rightAlignRenderer);
}
else { // game over
theprojectTable.getColumnModel().getColumn(theprojectModel.getColumnIndex("CurrentIteration")).setCellRenderer(rightAlignRenderer);
theprojectTable.getColumnModel().getColumn(theprojectModel.getColumnIndex("Score")).setCellRenderer(rightAlignRenderer);
theprojectTable.getColumnModel().getColumn(theprojectModel.getColumnIndex("TimeElapsed")).setCellRenderer(rightAlignRenderer);
theprojectTable.getColumnModel().getColumn(theprojectModel.getColumnIndex("TimeAllotted")).setCellRenderer(rightAlignRenderer);
}
theprojectTable.update(theprojectTable.getGraphics());
resetHeight();
}
...
double height = 0;
height += ((theprojectTable.getRowHeight() + (theprojectTable.getRowMargin() * 2)) * (theprojectTable.getRowCount() + 1));
height += theprojectTitlePane.getSize().getHeight();
