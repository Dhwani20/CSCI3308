...
import java.util.*;
public class PairProgramPegSigfreidoAction extends Action implements Cloneable
{
private Hashtable<Employee, Boolean> pegs;
private Hashtable<Employee, Boolean> sigfreidos;
...
public PairProgramPegSigfreidoAction()
{
pegs = new Hashtable<Employee, Boolean>();
sigfreidos = new Hashtable<Employee, Boolean>();
...
public Object clone() {
PairProgramPegSigfreidoAction cl = (PairProgramPegSigfreidoAction)(super.clone());
Hashtable<Employee, Boolean> clonedpegs = new Hashtable<Employee, Boolean>();
clonedpegs.putAll(pegs);
cl.pegs = clonedpegs;
Hashtable<Employee, Boolean> clonedsigfreidos = new Hashtable<Employee, Boolean>();
clonedsigfreidos.putAll(sigfreidos);
cl.sigfreidos = clonedsigfreidos;
...
all.addAll(getAllPegs());
all.addAll(getAllSigfreidos());
...
{
Vector<SSObject> all = new Vector<SSObject>();
all.addAll(getAllActivePegs());
all.addAll(getAllActiveSigfreidos());
...
{
Vector<SSObject> all = new Vector<SSObject>();
all.addAll(getAllInactivePegs());
all.addAll(getAllInactiveSigfreidos());
...
public Vector<Employee> getAllPegs()
{
Vector<Employee>a = new Vector<Employee>();
Enumeration<Employee> e = pegs.keys();
...
public Vector<Employee> getAllActivePegs()
{
Vector<Employee> a = new Vector<Employee>();
Enumeration<Employee> e = pegs.keys();
for(int i=0; i<pegs.size(); i++)
...
if((pegs.get(key)).booleanValue() == true)
{
a.add(key);
}
}
...
public Vector<Employee> getAllInactivePegs()
{
Vector<Employee> a = new Vector<Employee>();
Enumeration<Employee> e = pegs.keys();
for(int i=0; i<pegs.size(); i++)
{
Employee key = e.nextElement();
if((pegs.get(key)).booleanValue() == false)
{
a.add(key);
}
}
...
public boolean addPeg(Employee a)
{
if((pegs.containsKey(a)) ||( ((a instanceof SoftwareDeveloper) == false)) || (pegs.size() >= 1))
{
return false;
}
else
{
pegs.put(a, new Boolean(true));
return true;
}
}

public boolean removePeg(Employee a)
{
if(pegs.containsKey(a))
{
pegs.remove(a);
return true;
}
...
public boolean setPegActive(Employee a)
{
if(pegs.containsKey(a))
{
pegs.put(a, new Boolean(true));
...
public boolean setPegInactive(Employee a)
{
if(pegs.containsKey(a))
{
pegs.put(a, new Boolean(false));
return true;
}
return false;
}
public Vector<Employee> getAllSigfreidos()
{
Vector<Employee>a = new Vector<Employee>();
Enumeration<Employee> e = sigfreidos.keys();
...
public Vector<Employee> getAllActiveSigfreidos()
{
Vector<Employee> a = new Vector<Employee>();
Enumeration<Employee> e = sigfreidos.keys();
for(int i=0; i<sigfreidos.size(); i++)
{
Employee key = e.nextElement();
if((sigfreidos.get(key)).booleanValue() == true)
{
a.add(key);
}
}
...
public Vector<Employee> getAllInactiveSigfreidos()
{
Vector<Employee> a = new Vector<Employee>();
Enumeration<Employee> e = sigfreidos.keys();
for(int i=0; i<sigfreidos.size(); i++)
{
Employee key = e.nextElement();
if((sigfreidos.get(key)).booleanValue() == false)
{
a.add(key);
}
}
...
public boolean addSigfreido(Employee a)
{
if((sigfreidos.containsKey(a)) ||( ((a instanceof SoftwareDeveloper) == false)) || (sigfreidos.size() >= 1))
{
return false;
}
else
{
sigfreidos.put(a, new Boolean(true));
return true;
}
}

public boolean removeSigfreido(Employee a)
{
if(sigfreidos.containsKey(a))
{
sigfreidos.remove(a);
return true;
}
...
public boolean setSigfreidoActive(Employee a)
{
if(sigfreidos.containsKey(a))
{
sigfreidos.put(a, new Boolean(true));
...
public boolean setSigfreidoInactive(Employee a)
{
if(sigfreidos.containsKey(a))
{
sigfreidos.put(a, new Boolean(false));
return true;
}
return false;
}
...
// peg participants:
Hashtable<Employee, Boolean> newPegs = new Hashtable<Employee, Boolean>();
Iterator<Map.Entry<Employee, Boolean>> pegsIterator = pegs.entrySet().iterator();
while (pegsIterator.hasNext()) {
Map.Entry<Employee, Boolean> entry = pegsIterator.next();
Employee oldPeg = entry.getKey();
if (oldPeg instanceof SoftwareDeveloper) {
Employee newPeg = employeeRep.getSoftwareDeveloperStateRepository().get(((SoftwareDeveloper)oldPeg).getName());
Boolean activeStatus = pegs.get(oldPeg);
newPegs.put(newPeg, activeStatus);
}
}
pegs.clear();
pegs.putAll(newPegs);

// sigfreido participants:
Hashtable<Employee, Boolean> newSigfreidos = new Hashtable<Employee, Boolean>();
Iterator<Map.Entry<Employee, Boolean>> sigfreidosIterator = sigfreidos.entrySet().iterator();
while (sigfreidosIterator.hasNext()) {
Map.Entry<Employee, Boolean> entry = sigfreidosIterator.next();
Employee oldSigfreido = entry.getKey();
if (oldSigfreido instanceof SoftwareDeveloper) {
Employee newSigfreido = employeeRep.getSoftwareDeveloperStateRepository().get(((SoftwareDeveloper)oldSigfreido).getName());
Boolean activeStatus = sigfreidos.get(oldSigfreido);
newSigfreidos.put(newSigfreido, activeStatus);
}
}
sigfreidos.clear();
sigfreidos.putAll(newSigfreidos);
