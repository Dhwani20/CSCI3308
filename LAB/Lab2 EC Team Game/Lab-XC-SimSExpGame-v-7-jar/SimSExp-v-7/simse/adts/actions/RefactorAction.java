...
refactoringtools = new Hashtable<Tool, Boolean>();
releaseplans = new Hashtable<Artifact, Boolean>();
}

public Object clone() {
RefactorAction cl = (RefactorAction)(super.clone());
...
Hashtable<Tool, Boolean> clonedrefactoringtools = new Hashtable<Tool, Boolean>();
clonedrefactoringtools.putAll(refactoringtools);
...
all.addAll(getAllRefactoringTools());
...
all.addAll(getAllActiveRefactoringTools());
...
all.addAll(getAllInactiveRefactoringTools());
...
public Vector<Tool> getAllRefactoringTools()
{
Vector<Tool>a = new Vector<Tool>();
Enumeration<Tool> e = refactoringtools.keys();
...
public Vector<Tool> getAllActiveRefactoringTools()
{
Vector<Tool> a = new Vector<Tool>();
Enumeration<Tool> e = refactoringtools.keys();
for(int i=0; i<refactoringtools.size(); i++)
...
if((refactoringtools.get(key)).booleanValue() == true)
{
a.add(key);
}
}
...
public Vector<Tool> getAllInactiveRefactoringTools()
{
Vector<Tool> a = new Vector<Tool>();
Enumeration<Tool> e = refactoringtools.keys();
for(int i=0; i<refactoringtools.size(); i++)
{
Tool key = e.nextElement();
if((refactoringtools.get(key)).booleanValue() == false)
{
a.add(key);
}
}
...
public boolean addRefactoringTool(Tool a)
{
if((refactoringtools.containsKey(a)) ||( ((a instanceof RefactoringTool) == false)))
{
return false;
}
else
{
refactoringtools.put(a, new Boolean(true));
return true;
}
}

public boolean removeRefactoringTool(Tool a)
{
if(refactoringtools.containsKey(a))
{
refactoringtools.remove(a);
return true;
}
return false;
}

public boolean setRefactoringToolActive(Tool a)
{
if(refactoringtools.containsKey(a))
{
refactoringtools.put(a, new Boolean(true));
...
public boolean setRefactoringToolInactive(Tool a)
{
if(refactoringtools.containsKey(a))
{
refactoringtools.put(a, new Boolean(false));
return true;
}
return false;
}
...
// refactoringtool participants:
Hashtable<Tool, Boolean> newRefactoringTools = new Hashtable<Tool, Boolean>();
Iterator<Map.Entry<Tool, Boolean>> refactoringtoolsIterator = refactoringtools.entrySet().iterator();
while (refactoringtoolsIterator.hasNext()) {
Map.Entry<Tool, Boolean> entry = refactoringtoolsIterator.next();
Tool oldRefactoringTool = entry.getKey();
if (oldRefactoringTool instanceof RefactoringTool) {
Tool newRefactoringTool = toolRep.getRefactoringToolStateRepository().get(((RefactoringTool)oldRefactoringTool).getDescription());
Boolean activeStatus = refactoringtools.get(oldRefactoringTool);
newRefactoringTools.put(newRefactoringTool, activeStatus);
}
}
refactoringtools.clear();
refactoringtools.putAll(newRefactoringTools);
